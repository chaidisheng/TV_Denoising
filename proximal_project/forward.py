...
def rot_180(tensor):
	#return tf.reverse_v2(tensor, [1, 2])
...
def forward(x, regularizer):
	...
	#conv_b = get_bias([CONV_KERNEL_NUM])
	...
	#tf.summary.histogram('trade_off', trade_off)

	for k in range(60):
		conv = conv2d(z, D) # conv_w[k]
		conv = conv / (tf.abs(conv) + 1e-12)
		gradient = lambda_*conv2d(conv, tf.reshape(rot_180(D), [3, 3, 2, 1])) # conv_w[k]
		#tf.summary.histogram('gradient', gradient)
		z = prox_l2(z - trade_off*gradient, x, trade_off)
